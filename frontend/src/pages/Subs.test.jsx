// Subs → Invoice Paid: the read-only Job Log columns (Install Prog, Install Hrs,
// Budget) and the release-name hyperlink that opens the same hub modal the Job Log opens.
...
describe('Subs — Invoice Paid columns', () => {
    ...
    it('shows the Job Log install progress as its own read-only column', async () => {
        ...
        // Job, Rel, Job name, Description, Stage, Start install, Status, Install Prog, Install Hrs
        expect(cells[7]).toHaveTextContent('75%');
        expect(cells[8]).toHaveTextContent('12.50');
    ...
    it('passes through Job Log install hours between Install Prog and Budget', async () => {
        load([row()]);
        const cells = await cellsOf('Bldg C stairs');
        expect(cells[7]).toHaveTextContent('75%');
        expect(cells[8]).toHaveTextContent('12.50');
        expect(cells[9]).toHaveTextContent('$687.50');
        expect(screen.getByText('Install Hrs')).toBeInTheDocument();
    });

    it('computes Budget as Install Hrs × $55.00', async () => {
        load([row()]);
        // 12.5 hrs × $55 = $687.50
        expect((await cellsOf('Bldg C stairs'))[9]).toHaveTextContent('$687.50');
    });

    it('leaves Budget and Install Hrs blank when the release has no install hours', async () => {
...
describe('Subs — Est. Billable', () => {
    ...
    it('bills the install-progress share of the budget', async () => {
        ...
        expect(cells[10]).toHaveTextContent('$495.00');
    });
    ...
    it('reads job_comp with the percent sign already in it', async () => {
        ...
        expect(cells[8]).toHaveTextContent('34.90');
        expect(cells[9]).toHaveTextContent('$1,919.50');
        expect(cells[10]).toHaveTextContent('$1,727.55');
    });

    it('reads a decimal progress', async () => {
        load([row({ job_comp: '12.5%', install_hrs: 10 })]);
        expect((await cellsOf('Bldg C stairs'))[10]).toHaveTextContent('$68.75');
    });

    it('clamps over-100 progress to the full budget', async () => {
        load([row({ job_comp: '110%', install_hrs: 10 })]);
        expect((await cellsOf('Bldg C stairs'))[10]).toHaveTextContent('$550.00');
    });
    ...
    it('bills nothing at 0% but still shows the dollar figure', async () => {
        ...
        expect((await cellsOf('Bldg C stairs'))[10]).toHaveTextContent('$0.00');
    });
    ...
    it('blanks — rather than $0 — when progress or hours are unknown', async () => {
        ...
        expect((await cellsOf('Bldg C stairs'))[10]).toHaveTextContent('—');
        // No hours: no budget to take a share of.
        expect((await cellsOf('Bldg D rails'))[10]).toHaveTextContent('—');
    });
});
