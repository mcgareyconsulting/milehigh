...
function DashboardPlaceholder() {
  ...
  // Only restart loop when container size or item count changes, not on color updates
  useEffect(() => {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 w-full">
      <div
        ref={containerRef}
        className="relative flex-1 min-h-0 w-full overflow-hidden bg-transparent"
      >
        {/* Transparent logo as background watermark */}
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <img
            src="/logo.jpg"
            alt=""
            className="max-w-[min(60vw,400px)] max-h-[min(50vh,400px)] w-auto h-auto object-contain opacity-30"
            aria-hidden
          />
        </div>

        {/* Bananas floating on top */}
        {items.map((item, i) => (
          <div
            key={item.id}
            ref={(el) => {
