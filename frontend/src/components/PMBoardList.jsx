 ...
 * @milehigh-header
 * schema_version: 3
 * purpose: Renders the PM Kanban as the 6 Trello-list columns (mirroring the physical board) of release cards, plus read-only per-installer-team columns and vendor pick-up cards in the Shipping planning column.
 * exports:
 *   PMBoardList: Trello-mirroring board — the 6 Trello-list columns (drag-to-stage) followed by one read-only column per configured installer team; releases group by their backend-computed trello_list, and a release with a hard date + assigned installer ALSO renders as a copy in its installer column.
 * imports_from: [react, react-router-dom, ../services/jobsApi, ./PMBoardCardModal, ./PickupCardModal]
 ...
 *   - Installer columns come from /brain/installer-teams (Config.INSTALLER_TEAMS) and use the Gantt lane palette so List and Timeline colors match. They are read-only: not drop targets, cards are not draggable.
 ...
 *   - Dropping a release on a stage column sets its stage to that column's floor stage; pick-up cards are display-only.
 * updated_by_agent: 2026-05-26 (installer-team columns in list view)
 */
            ...
            <div className="flex gap-2 flex-1 min-h-0">
                {allColumns.map((column) => {
                    ...
                    const listJobs = isInstaller
                        ...
                        : (jobsByList[list] || []);
                    const listPickups = isInstaller ? [] : (pickupsByList[list] || []);
