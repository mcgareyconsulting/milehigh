...
export default function BBChatWidget({ enabled, isAdmin }) {
    ...
    return (
        <>
            ...
            {/* Panel */}
            {open && (
                <div className="fixed z-50 bottom-20 right-4 left-4 sm:left-auto sm:w-[26rem] h-[70vh] sm:h-[32rem] max-h-[80vh] bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-slate-600 flex flex-col overflow-hidden">
                    {showAccess ? (
                        <AccessPanel onClose={() => setShowAccess(false)} />
                    ) : (
                        <>
                            {/* Header */}
                            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-slate-600 bg-gradient-to-r from-accent-500 to-accent-600 text-white">
                            ...
                            <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 space-y-3">
                                ...
                                {messages.map((m, i) => (
                                    <div key={i} className={m.role === 'user' ? 'flex justify-end' : 'flex justify-start'}>
                                        <div className={`max-w-[85%] rounded-2xl px-3 py-2 text-sm whitespace-pre-wrap break-words ${m.role === 'user'
                            ...
                            {/* Input */}
                            <div className="border-t border-gray-200 dark:border-slate-600 p-2 flex items-end gap-2">
                                <textarea
                                    value={input}
