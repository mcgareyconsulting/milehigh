...
class JobsApi {
    ...
    // ---- Stash sessions (Thursday review meeting flow) ----

    async getActiveStashSession() {
        try {
            const response = await axios.get(`${API_BASE_URL}/brain/stash-sessions/active`);
            return response.data;
        } catch (error) {
            throw this._handleError(error, 'Failed to fetch active stash session');
        }
    }

    async startStashSession() {
        try {
            const response = await axios.post(`${API_BASE_URL}/brain/stash-sessions`);
            return response.data;
        } catch (error) {
            throw this._handleError(error, 'Failed to start stash session');
        }
    }

    async stashChange(sessionId, { job, release, field, newValue }) {
        try {
            const response = await axios.post(
                `${API_BASE_URL}/brain/stash-sessions/${sessionId}/changes`,
                { job, release, field, new_value: newValue }
            );
            return response.data;
        } catch (error) {
            throw this._handleError(error, 'Failed to stash change');
        }
    }

    async removeStashedChange(sessionId, changeId) {
        try {
            const response = await axios.delete(
                `${API_BASE_URL}/brain/stash-sessions/${sessionId}/changes/${changeId}`
            );
            return response.data;
        } catch (error) {
            throw this._handleError(error, 'Failed to remove stashed change');
        }
    }

    async getStashPreview(sessionId) {
        try {
            const response = await axios.get(
                `${API_BASE_URL}/brain/stash-sessions/${sessionId}/preview`
            );
            return response.data;
        } catch (error) {
            throw this._handleError(error, 'Failed to fetch stash preview');
        }
    }

    async applyStashSession(sessionId) {
        try {
            const response = await axios.post(
                `${API_BASE_URL}/brain/stash-sessions/${sessionId}/apply`
            );
            return response.data;
        } catch (error) {
            throw this._handleError(error, 'Failed to apply stash session');
        }
    }

    async discardStashSession(sessionId) {
        try {
            const response = await axios.post(
                `${API_BASE_URL}/brain/stash-sessions/${sessionId}/discard`
            );
            return response.data;
        } catch (error) {
            throw this._handleError(error, 'Failed to discard stash session');
        }
    }
