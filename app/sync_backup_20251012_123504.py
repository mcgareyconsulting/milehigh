    ...
    with SyncContext("trello_webhook", sync_op.operation_id):
        try:
            ...
            if rec:
                ...
                # Check if this is a new card creation event
                if event_info.get("event") == "card_created":
                    print(f"Hey! Card added: {card_data.get('name')} (ID: {card_id})")
                    logger.info(
                        "New card created - processing",
                        operation_id=sync_op.operation_id,
                        card_id=card_id,
                        trello_name=card_data.get("name")
                    )
                    ...
                    # Step 1: Extract job-release identifier from card name
                    card_name = card_data.get("name", "")
                    identifier = extract_identifier(card_name)
                    
                    if not identifier:
                        print(f"No valid job-release identifier found in card name: {card_name}")
                        logger.warning(
                            "No valid identifier found in new card",
                            operation_id=sync_op.operation_id,
                            card_id=card_id,
                            card_name=card_name
                        )
                        update_sync_operation(sync_op.operation_id, status=SyncStatus.SKIPPED, error_type="NoValidIdentifier")
                        return
                    
                    # Parse job and release from identifier (e.g., "123-456" -> job=123, release="456")
                    try:
                        ...
                        print(f"Could not parse job-release from identifier: {identifier}")
                        logger.warning(
                        ...
                        update_sync_operation(sync_op.operation_id, status=SyncStatus.SKIPPED, error_type="ParseError")
                        return
                    
                    # Step 2: Check if this job-release already exists in DB
                    existing_job = Job.query.filter_by(job=job, release=release).first()
                    if existing_job:
                        print(f"Job-release {identifier} already exists in DB - ignoring new card")
                        logger.info(
                        ...
                        update_sync_operation(sync_op.operation_id, status=SyncStatus.SKIPPED, error_type="AlreadyExists")
                        return
                    
                    # Step 3: Check if job-release exists in Excel using existing function
                    excel_index, excel_row = get_excel_row_and_index_by_identifiers(job, release)
                    
                    if excel_row is None:
                        print(f"Job-release {identifier} not found in Excel - ignoring new card")
                        logger.info(
                        ...
                        update_sync_operation(sync_op.operation_id, status=SyncStatus.SKIPPED, error_type="NotInExcel")
                        return
                    
                    # Step 4: Create new DB record with combined data
                    print(f"Creating new DB record for {identifier} with Excel data")
                    logger.info(
                        "Creating new DB record with Excel data",
                        operation_id=sync_op.operation_id,
                        card_id=card_id,
                        identifier=identifier
                    )
                    
                    # Create new Job record using existing seed logic
                    from app.seed import safe_truncate_string
                    ...
                    print(f"Successfully created new job record for {identifier}")
                    logger.info(
                    ...
                    update_sync_operation(sync_op.operation_id, status=SyncStatus.COMPLETED, records_created=1)
                    return
                else:
                    logger.info(
