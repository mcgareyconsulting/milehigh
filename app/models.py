...
class StashSession(db.Model):
    """
    A 'stash' window during the Thursday review meeting.

    While a session is active, admin UI edits on the Job Log are queued as
    StashedJobChange rows instead of being applied. At the end of the meeting
    the session is applied (batch-replayed through normal business logic) or
    discarded.

    Globally at most one session may have status='active' — enforced by a
    partial unique index created in the migration.
    """
    __tablename__ = "stash_sessions"
    id = db.Column(db.Integer, primary_key=True)
    started_by_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
    status = db.Column(db.String(16), nullable=False, default='active')
    # 'active' | 'applied' | 'partial' | 'discarded'
    started_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    ended_at = db.Column(db.DateTime, nullable=True)

    started_by = db.relationship('User', lazy='select')
    changes = db.relationship(
        'StashedJobChange', backref='session', lazy='dynamic',
...
class StashedJobChange(db.Model):
    """
    One queued edit to a single (job, release, field) cell during a stash session.

    Upserted on (session_id, job, release, field): a later edit to the same cell
    overwrites `new_value` (latest wins) but preserves the original `baseline_value`
    captured the first time the cell was edited in the session.
    """
    __tablename__ = "stashed_job_changes"
    id = db.Column(db.Integer, primary_key=True)
    session_id = db.Column(db.Integer, db.ForeignKey('stash_sessions.id'), nullable=False, index=True)
    job = db.Column(db.Integer, nullable=False)
    release = db.Column(db.String(16), nullable=False)
    field = db.Column(db.String(32), nullable=False)
    # 'stage' | 'fab_order' | 'notes' | 'job_comp' | 'invoiced' | 'start_install'
    baseline_value = db.Column(db.JSON, nullable=True)
    new_value = db.Column(db.JSON, nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)
    applied_at = db.Column(db.DateTime, nullable=True)
    status = db.Column(db.String(16), default='pending', nullable=False)
    # 'pending' | 'applied' | 'conflict' | 'failed' | 'no_op'
    error = db.Column(db.Text, nullable=True)

    __table_args__ = (
        db.UniqueConstraint('session_id', 'job', 'release', 'field', name='_stash_change_uc'),
    )
