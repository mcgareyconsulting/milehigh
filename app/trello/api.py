...
def create_trello_card_from_excel_data(excel_data, list_name=None):
    ...
    try:
        ...
        if excel_data.get("Install HRS"):
            ...
            installation_duration = calculate_installation_duration(
                install_hrs, num_guys
            )
            if installation_duration is not None:
                description_parts.append(
                    f"**Installation Duration:** {installation_duration} days"
                )
        ...
        if excel_data.get("PM") and excel_data.get("BY"):
            description_parts.append(
                f"**Team:** PM: {excel_data['PM']} / BY: {excel_data['BY']}"
            )
        ...
        if (
            notes_value is not None
            and not pd.isna(notes_value)
    ...
    try:
        ...
        if len(card_data) == 1:
            ...
            position_updates.append(
                {"card_id": card_data[0]["card_id"], "new_pos": "top"}
            )
        else:
...
def get_card_attachments_by_job_release(job_number, release_number):
    ...
    try:
        ...
        query = {"key": cfg.TRELLO_API_KEY, "token": cfg.TRELLO_TOKEN}
...
def update_mirror_card_date_range(trello_card_id, start_date, install_hrs):
    ...
    try:
        ...
        if not mirror_short_link:
            return {"success": False, "error": "No fileName found in attachment data"}
...
def update_card_date_range(card_short_link, start_date, due_date):
    """
    Push a release's install window to its mirror (installer) card as a range bar:
    start = start_install, due = comp_eta.

    BUG-25 ruling (Bill, 2026-09-16): the primary card is Due-only, but the mirror card keeps
    BOTH dates — collapsing its due onto the install day would kill the range on the Timeline.

    Args:
        card_short_link (str): The card's short link (from fileName)
        start_date (datetime.date): The install day (start_install)
        due_date (datetime.date): The completion day (comp_eta)
...
def add_procore_link(card_id, procore_url, link_name=None):
    ...
    try:
        ...
        return {
            ...
            "error": f"HTTP error: {http_err}",
            "response": (
                http_err.response.text if hasattr(http_err.response, "text") else None
            ),
        }
    except Exception as err:
        logger.error(
