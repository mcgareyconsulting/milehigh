...
class ProcoreAPI:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if r.status_code == 400:
                    raise requests.HTTPError(f"400 from Procore: {r.text}", response=r)
    ...
    def get_user_information_by_id(self, user_id: int) -> List[Dict]:
        return self._get(
            f"/rest/v1.3/companies/{cfg.PROD_PROCORE_COMPANY_ID}/users/{user_id}"
        )
    ...
    def get_sub_filters_by_project_id(self, project_id: int) -> List[Dict]:
        return self._get(
            f"/rest/v1.0/projects/{project_id}/submittals/filter_options/status_id"
        )
    ...
    def get_submittals_for_drafting_workload(self, project_id: int) -> List[Dict]:
        ...
        return self._get(
            f"/rest/v1.1/projects/{project_id}/submittals?filters[type][]=Drafting Release Review&filters[type][]=Submittal for GC  Approval&filter[type][]=Submittal for GC Approval&filters[status_id]=203238"
        )

    def get_submittals_statuses(self) -> List[Dict]:
        return self._get(
            f"/rest/v1.0/companies/{cfg.PROD_PROCORE_COMPANY_ID}/submittal_statuses"
        )

    def update_submittal_status(
        self, project_id: int, submittal_id: int, status_id: int
    ) -> Dict:
        ...
        if status_id not in VALID_SUBMITTAL_STATUS_IDS:
            raise ValueError(
                f"Invalid status_id {status_id}; must be one of {sorted(VALID_SUBMITTAL_STATUS_IDS)}"
            )
        return self._patch(
    ...
    def list_project_webhooks(self, project_id: int, namespace: str) -> List[Dict]:
        ...
        response = self._get(
            f"/rest/v2.0/companies/{cfg.PROD_PROCORE_COMPANY_ID}/projects/{project_id}/webhooks/hooks?namespace={namespace}"
        )
    ...
    def create_project_webhook(
        self, project_id: int, name: str, event_type: str
    ) -> Dict:
        data = {
        ...
        return self._post(
            f"/rest/v2.0/companies/{cfg.PROD_PROCORE_COMPANY_ID}/projects/{project_id}/webhooks/hooks",
            data,
        )

    def create_webhook_trigger(
        self, project_id: int, hook_id: int, event_type: str
    ) -> Dict:
        data = {
        ...
        return self._post(
            f"/rest/v2.0/companies/{cfg.PROD_PROCORE_COMPANY_ID}/projects/{project_id}/webhooks/hooks/{hook_id}/triggers",
            data,
        )
    ...
    def get_webhook_details(self, project_id: int, hook_id: int) -> Dict:
        ...
        return self._get(
            f"/rest/v2.0/companies/{cfg.PROD_PROCORE_COMPANY_ID}/projects/{project_id}/webhooks/hooks/{hook_id}"
        )
    ...
    def get_deliveries(
        self, company_id: int, project_id: int, hook_id: int
    ) -> List[Dict]:
        """
        ...
        try:
            response = self._get(
                f"/rest/v2.0/companies/{company_id}/projects/{project_id}/webhooks/hooks/{hook_id}/deliveries"
            )
            # Procore API returns {"data": [...]} format, extract the list
            if isinstance(response, dict) and "data" in response:
                return response["data"] if response["data"] else []
