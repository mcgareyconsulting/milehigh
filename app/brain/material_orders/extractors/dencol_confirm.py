...
_UNIT_PRICE_RE = re.compile(r"\$\s*([\d,]+\.\d{2})\s*EA", re.IGNORECASE)
_DOLLAR_RE = re.compile(r"\$\s*([\d,]+\.\d{2})")
_ORDER_NO_RE = re.compile(r"Order\s*#\s*:?\s*(\d+)", re.IGNORECASE)
...
def _parse_table(text):
    ...
    for i, line in enumerate(lines):
        ...
        # Unit/extended price sit on the nearest preceding "... $u EA $ext ..." line.
        unit_price = extended_price = None
        for prev in reversed(lines[max(0, i - 3):i]):
            um = _UNIT_PRICE_RE.search(prev)
            if not um:
                continue
            unit_price = _money(um.group(1))
            after = _DOLLAR_RE.search(prev, um.end())
            extended_price = _money(after.group(1)) if after else None
            break
...
def _confirm_text(record):
    ...
    for att in base.pdf_attachments(record):
        ...
        if _DESC_RE.search(text) and _UNIT_PRICE_RE.search(text):
            return text
    return ""
