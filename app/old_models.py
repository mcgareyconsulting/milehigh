...
class ProcoreWebhookEvents(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint(
            "resource_id", "project_id", "event_type", name="_procore_webhook_unique"
        ),
    )
...
class ProcoreSubmittal(db.Model):
    ...
    ball_in_court = db.Column(
        db.String(255)
    )  # Increased to handle multiple assignees (comma-separated)
    ...
    was_multiple_assignees = db.Column(
        db.Boolean, default=False
    )  # Track if submittal was previously in multiple-assignee state
    last_bic_update = db.Column(
        db.DateTime, nullable=True
    )  # Track the date and time of the most recent ball in court update
...
class SyncOperation(db.Model):
    ...
    )  # 'trello_webhook', etc. (onedrive_poll removed)
    ...
    )  # 'trello', 'system' (onedrive removed)
...
class JobChangeLog(db.Model):
    ...
    )  # "state_change", "field_change" (future)
    ...
    field_name = db.Column(
        db.String(100)
    )  # e.g., "fitup_comp", "state" for state changes
