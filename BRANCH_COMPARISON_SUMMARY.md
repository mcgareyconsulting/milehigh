...
- **Event Tracking**:
  ...
  - **excel_poller_teardown**: Uses `SubmittalEvents` with `action`, `payload`, `payload_hash` pattern (similar to ReleaseEvents)
...
- **REMOVED**: `JobChangeLog` (replaced by `ReleaseEvents`)

#### 3. **Submittals Model Changes**:
...
- **CHANGED**: `/drafting-work-load` now filters by status:
  ...
  submittals = Submittals.query.filter(
      Submittals.status.in_(['Open', 'Draft'])
  ).all()
  ```
...
1. **Event-Driven System**: Moved from simple webhook tracking to full event sourcing with `SubmittalEvents` and `ReleaseEvents`
...
4. **Aging Report Support** (`Submittals.get_last_ball_in_court_update_time()`) - Useful feature
...
1. **Database Schema**: Requires migration for new models (`SubmittalEvents`, `ReleaseEvents`, `Outbox`, `User`, `SystemLogs`)
2. **Removed Fields**: `due_date`, `last_bic_update` in `Submittals`
