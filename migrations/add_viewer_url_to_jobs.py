"""
Add the viewer_url column to the jobs table.
...
def migrate(database_url: str = None) -> bool:
    """Perform the migration, adding viewer_url to jobs if needed."""
    ...
    try:
        if column_exists(engine, "jobs", "viewer_url"):
            print("✓ Column 'viewer_url' already exists on 'jobs'. Nothing to do.")
            return True

        print("Adding column 'viewer_url' to 'jobs' table...")
        with engine.begin() as conn:
            conn.execute(text("ALTER TABLE jobs ADD COLUMN viewer_url VARCHAR(512)"))

        # Re-check to confirm
        if column_exists(engine, "jobs", "viewer_url"):
            print("✓ Successfully added 'viewer_url' column to 'jobs'.")
            return True
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Add viewer_url column to jobs table.")
    parser.add_argument(
        "--database-url",
