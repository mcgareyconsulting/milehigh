"""
Create the tm_tickets table — Time & Material tickets ingested from legacy paper
scans/photos/PDFs via Claude vision, reviewed by a human before confirmation.

This migration only creates the new table and its indexes; nothing existing is
altered and nothing is backfilled. The FK to releases(id) briefly takes a
SHARE ROW EXCLUSIVE lock on releases while the constraint is created, which the
lock_timeout + retry loop below keeps harmless on a live database.

Usage:
    python migrations/add_tm_tickets.py
    python migrations/add_tm_tickets.py --database-url postgresql://...
...
# Idempotent DDL — works on both Postgres and modern SQLite. JSON degrades to
# TEXT affinity on SQLite, which is what SQLAlchemy's JSON type expects there.
_TM_TICKETS_TABLE = """
    CREATE TABLE IF NOT EXISTS tm_tickets (
        ...
        source_storage_key VARCHAR(128),
        source_filename VARCHAR(255),
        source_media_type VARCHAR(64),
...
"""
_INDEXES = (
    "CREATE INDEX IF NOT EXISTS ix_tm_tickets_status ON tm_tickets (status)",
    "CREATE INDEX IF NOT EXISTS ix_tm_tickets_release_id ON tm_tickets (release_id)",
    "CREATE INDEX IF NOT EXISTS ix_tm_tickets_job ON tm_tickets (job)",
)
...
def _migrate_postgres(engine) -> bool:
    with engine.connect().execution_options(isolation_level="AUTOCOMMIT") as conn:
        ...
        try:
            _run_with_retry(
                conn,
                _TM_TICKETS_TABLE.format(pk="SERIAL PRIMARY KEY", false="FALSE"),
                "tm_tickets table",
            )
            for sql in _INDEXES:
                _run_with_retry(conn, sql, sql.split(" ON ")[0].split("EXISTS ")[-1])
        except OperationalError as exc:
...
def _migrate_sqlite(engine) -> bool:
    ...
    with engine.begin() as conn:
        conn.execute(text(_TM_TICKETS_TABLE.format(pk="INTEGER PRIMARY KEY AUTOINCREMENT", false="0")))
        for sql in _INDEXES:
            conn.execute(text(sql))
        print("✓ tm_tickets table + indexes")
    return True
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create the tm_tickets table.")
    parser.add_argument(
        "--database-url",
