"""
Add the stage column to the jobs table.

Usage:
    python migrations/add_stage_column_to_jobs.py
...
def migrate(database_url: str = None) -> bool:
    """Perform the migration, adding stage to jobs if needed."""
    ...
    try:
        if column_exists(engine, "jobs", "stage"):
            print("✓ Column 'stage' already exists on 'jobs'. Nothing to do.")
            return True
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Add stage column to jobs table.")
    parser.add_argument(
        "--database-url",
