"""
Add shipping-planning fields to material_orders: `order_kind`, `shipping_status`, `ready_at`.

Two new supplier-email shapes now land as MaterialOrder rows that carry no itemized
parts — a galvanizing status notification (AZZ, one row upserted per AZZ Job #) and a
DenCol stock restock (no release). `order_kind` ('material' | 'galvanizing' | 'stock')
lets the shipping-planning lane source and style them; `shipping_status`
('planning' | 'complete') is their bring-it-in lifecycle. Plain material rows keep
order_kind='material' and leave shipping_status null (their lifecycle stays the
ordered/received `status`).

This migration only adds the columns; nothing is backfilled (the server_default on
order_kind stamps existing rows 'material' at read time via the ORM default, and new
rows get it from the DB default below).

Usage:
    python migrations/add_order_kind_shipping_status_to_material_orders.py
    python migrations/add_order_kind_shipping_status_to_material_orders.py --database-url postgresql://...

Safety properties (Postgres) — mirrors migrations/add_start_install_to_dwl.py:
  - Every statement is idempotent (`ADD COLUMN IF NOT EXISTS`), so it needs NO schema
    reflection — no second pooled connection can self-block on our own lock.
  ...
  - ADD COLUMN stays metadata-only (nullable / constant default) so it's instant.
"""
...
# (column, type + constant default) — both metadata-only, so ADD COLUMN is instant.
COLUMNS = [
    ("order_kind", "VARCHAR(16) DEFAULT 'material'"),
    ("shipping_status", "VARCHAR(16)"),
    ("ready_at", "DATE"),
]
...
def _migrate_sqlite(engine) -> bool:
    ...
    # ADD COLUMN IF NOT EXISTS, so guard each column by inspection.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Add order_kind + shipping_status columns to material_orders."
    )
    parser.add_argument(
        "--database-url",
