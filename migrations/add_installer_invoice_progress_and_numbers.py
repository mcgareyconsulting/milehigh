"""
Add installer invoice progress % and invoice number(s) to the releases table.

Used by the admin Subs → Invoice Paid page (Katie/Lexi):
  - installer_invoice_progress  — 0–100 fillable progress toward the sub invoice
  - installer_invoice_numbers    — multi-line free text for one or more invoice numbers

Distinct from `invoiced` (MHMW customer billing) and from `installer_invoice_paid`
(the yes/no "Invoiced complete" toggle).

Usage:
    python migrations/add_installer_invoice_progress_and_numbers.py
    python migrations/add_installer_invoice_progress_and_numbers.py --database-url postgresql://...
...
Safety properties (Postgres) — never freeze the table:
  ...
  - Metadata-only ADD COLUMN (nullable, no volatile default) — instant.
    ...
    with engine.connect().execution_options(isolation_level="AUTOCOMMIT") as conn:
        ...
        columns = [
            (
                ...
                "ADD COLUMN IF NOT EXISTS installer_invoice_progress INTEGER",
                "releases.installer_invoice_progress",
            ),
            (
                "ALTER TABLE releases "
                "ADD COLUMN IF NOT EXISTS installer_invoice_numbers TEXT",
                "releases.installer_invoice_numbers",
            ),
        ]
        for sql, label in columns:
            try:
...
def _migrate_sqlite(engine) -> bool:
    ...
    with engine.begin() as conn:
        if "installer_invoice_progress" not in existing:
            conn.execute(text(
                "ALTER TABLE releases "
                "ADD COLUMN installer_invoice_progress INTEGER"
            ))
            print("✓ releases.installer_invoice_progress")
        else:
            print("releases.installer_invoice_progress already exists, skipping")

        if "installer_invoice_numbers" not in existing:
            conn.execute(text(
                "ALTER TABLE releases "
                "ADD COLUMN installer_invoice_numbers TEXT"
            ))
            print("✓ releases.installer_invoice_numbers")
        else:
            print("releases.installer_invoice_numbers already exists, skipping")
    return True
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Add installer_invoice_progress and installer_invoice_numbers "
            "to the releases table."
        )
    )
    parser.add_argument(
        "--database-url",
