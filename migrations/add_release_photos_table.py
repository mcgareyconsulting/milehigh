"""
Create the release_photos table for the release photo attachments feature.
...
Usage:
    ...
    ENVIRONMENT=sandbox python migrations/add_release_photos_table.py

    # Skip the interactive confirmation when targeting sandbox/production:
    ENVIRONMENT=sandbox python migrations/add_release_photos_table.py --yes

    # Or pass an explicit database URL (overrides ENVIRONMENT):
    python migrations/add_release_photos_table.py --database-url postgresql://...
...
def migrate(database_url: str) -> bool:
    ...
    try:
        if table_exists(engine, "release_photos"):
            print("✓ Table 'release_photos' already exists. Nothing to do.")
            return True

        print("Creating table 'release_photos'...")
        ...
        with engine.begin() as conn:
            ...
            conn.execute(text(
                "CREATE INDEX IF NOT EXISTS ix_release_photos_release_id "
                "ON release_photos (release_id)"
            ))

        if table_exists(engine, "release_photos"):
            print("✓ Successfully created 'release_photos' table.")
            return True
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create release_photos table.")
    parser.add_argument(
        "--database-url",
