"""
Add `phone` to the `subcontractors` table — the contact phone number on a
subcontractor account (T3, from the client's contact sheet).

Nullable with no default, so the ALTER is metadata-only and instant. Nothing is
backfilled.

Usage:
    python migrations/add_phone_to_subcontractors.py
    python migrations/add_phone_to_subcontractors.py --database-url postgresql://...

Safety properties (Postgres) — see migrations/README.md: idempotent
`ADD COLUMN IF NOT EXISTS` (no schema reflection under a lock), one AUTOCOMMIT
connection, `lock_timeout` + retry with backoff.
"""
...
def _migrate_postgres(engine) -> bool:
    ...
    with engine.connect().execution_options(isolation_level="AUTOCOMMIT") as conn:
        ...
        try:
            _run_with_retry(
                conn,
                "ALTER TABLE subcontractors ADD COLUMN IF NOT EXISTS phone VARCHAR(32)",
...
def _migrate_sqlite(engine) -> bool:
    ...
    with engine.begin() as conn:
        if "phone" not in existing:
            conn.execute(text("ALTER TABLE subcontractors ADD COLUMN phone VARCHAR(32)"))
            print("OK subcontractors.phone")
        else:
            print("subcontractors.phone already exists, skipping")
    return True
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Add the phone column to the subcontractors table."
    )
    parser.add_argument(
        "--database-url",
