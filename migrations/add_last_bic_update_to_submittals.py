"""
Add last_bic_update column to procore_submittals table.

Usage:
    python migrations/add_last_bic_update_to_submittals.py
...
def migrate(database_url: str = None) -> bool:
    """Perform the migration, adding last_bic_update column if needed."""
    ...
    try:
        ...
        last_bic_update_exists = column_exists(engine, "procore_submittals", "last_bic_update")

        if last_bic_update_exists:
            print("✓ Column 'last_bic_update' already exists. Nothing to do.")
            return True

        # Now start transaction only if we need to make changes
        with engine.begin() as conn:
            print("Adding column 'last_bic_update' to 'procore_submittals' table...")
            # Check if we're using PostgreSQL or SQLite
            db_url_lower = str(db_url).lower()
            if "postgresql" in db_url_lower or "postgres" in db_url_lower:
                conn.execute(text("ALTER TABLE procore_submittals ADD COLUMN last_bic_update TIMESTAMP"))
            else:
                # SQLite uses DATETIME type
                conn.execute(text("ALTER TABLE procore_submittals ADD COLUMN last_bic_update DATETIME"))
            print("✓ Successfully added 'last_bic_update' column.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Add last_bic_update column to procore_submittals table.")
    parser.add_argument(
        "--database-url",
