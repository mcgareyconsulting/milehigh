"""
Add due_date column to procore_submittals table.

Usage:
    python migrations/add_due_date_to_submittals.py
...
def migrate(database_url: str = None) -> bool:
    """Perform the migration, adding due_date column if needed."""
    ...
    try:
        ...
        column_exists_check = column_exists(engine, "procore_submittals", "due_date")

        if column_exists_check:
            print("✓ Column 'due_date' already exists. Nothing to do.")
            return True

        # Now start transaction only if we need to make changes
        with engine.begin() as conn:
            # Add due_date column if it doesn't exist
            print("Adding column 'due_date' to 'procore_submittals' table...")
            
            # Check if we're using PostgreSQL
            db_url_lower = str(db_url).lower()
            if "postgresql" in db_url_lower or "postgres" in db_url_lower:
                # PostgreSQL: Add column as nullable DATE
                conn.execute(text("""
                    ALTER TABLE procore_submittals 
                    ADD COLUMN due_date DATE
                """))
                print("✓ Successfully added 'due_date' column (nullable DATE).")
            else:
                # SQLite: Add column as nullable (SQLite doesn't have a DATE type, uses TEXT)
                conn.execute(text("ALTER TABLE procore_submittals ADD COLUMN due_date DATE"))
                print("✓ Successfully added 'due_date' column (nullable DATE).")
                print("⚠ Note: SQLite stores dates as TEXT. Values should be in ISO format (YYYY-MM-DD).")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Add due_date column to procore_submittals table.")
    parser.add_argument(
        "--database-url",
