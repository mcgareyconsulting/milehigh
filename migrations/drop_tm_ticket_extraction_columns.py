"""
Drop the vestigial legacy-paper-extraction columns from tm_tickets.

These columns were built for a paper-ticket-photo-and-extract path that was
dropped from v1 scope before any environment saw real ticket data on it
(`docs/feature-catalog.md` A1: "Paper ticket ingestion — DROPPED. Origination
path only."). The extraction service (app/brain/tm/extract.py), its storage
helper (app/brain/tm/storage.py), and service.create_from_upload have all been
removed from the codebase; this migration catches the schema up to match.
Native tickets always left these columns null, so nothing is backfilled or lost.

Columns dropped: raw_extraction, extract_model, extract_error,
source_storage_key, source_filename, source_media_type, source_record_id,
uploaded_by.

Usage:
    python migrations/drop_tm_ticket_extraction_columns.py
    python migrations/drop_tm_ticket_extraction_columns.py --database-url postgresql://...

Safety properties (Postgres) — mirrors migrations/add_start_install_to_dwl.py:
  - Every statement is idempotent (`DROP COLUMN IF EXISTS`), so it needs NO
    schema reflection.
  ...
  - `lock_timeout` makes a blocked ALTER FAIL FAST instead of queueing behind
    live traffic, then auto-retries with backoff.
"""
...
def _migrate_postgres(engine) -> bool:
    with engine.connect().execution_options(isolation_level="AUTOCOMMIT") as conn:
        ...
        if conn.execute(text("SELECT to_regclass('tm_tickets')")).scalar() is None:
            print("✗ Table 'tm_tickets' does not exist. Nothing to do.")
            return False
        ...
        try:
            for column in DROPPED_COLUMNS:
                _run_with_retry(
                    conn,
                    f"ALTER TABLE tm_tickets DROP COLUMN IF EXISTS {column}",
                    f"tm_tickets.{column} dropped",
                )
        except OperationalError as exc:
            if _is_lock_timeout(exc):
                print(
                    f"✗ Gave up after {LOCK_RETRIES} attempts: could not get the lock on "
                    "'tm_tickets' — the table is under sustained load. Some columns may "
                    "already be dropped (each statement is independent); re-run to finish.\n"
                    "  Find an idle-in-transaction blocker:\n"
...
def _migrate_sqlite(engine) -> bool:
    ...
    # concern. DROP COLUMN has no IF EXISTS clause here, so guard by inspection.
    ...
    with engine.begin() as conn:
        for column in DROPPED_COLUMNS:
            if column in existing:
                conn.execute(text(f"ALTER TABLE tm_tickets DROP COLUMN {column}"))
                print(f"✓ tm_tickets.{column} dropped")
            else:
                print(f"tm_tickets.{column} already absent, skipping")
    return True
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Drop the vestigial legacy-paper-extraction columns from tm_tickets."
    )
    parser.add_argument(
        "--database-url",
