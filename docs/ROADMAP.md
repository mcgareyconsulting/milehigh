...
- 2026-08-21 · decision · src daniel — the **ASAP drop moves to Install Start too**: ASAP red is a colour, so it rides through the ship stages with the rest. Was `Ship Complete` rank ≥ 14 since the ASAP-mode build
- 2026-08-21 · build · src — — BUG-11 shipped: `COLOR_DUMP_STAGE` / `is_at_or_past_color_dump` (Install Start and later, Hold excluded) replace the ship-stage wash. `apply_shipping_stage_date_discipline` now routes two rules with two triggers; hard dates return early at the ship stages. The frontend's own optimistic copy of the rule (`JobsTableRow`, `StartInstallEditor`) moved to a shared `isAtOrPastColorDump` helper in `constants/stages.js` — without it the color still vanished on screen regardless of the backend. Suite green (1291). The ASAP drop shares the predicate (`asap_dropped_on_install_start`); it fires before the colour dump, so a hard-dated release clears the flag and the colour in one stage change, and an ASAP release with no hard date still drops cleanly (the colour dump no-ops without a hard date)

**Watch item from the ASAP move:** the 2-per-PM ASAP cap
(`app/brain/job_log/routes.py:1786`) counts every row with
`start_install_asap=True` with no stage filter, so an ASAP release now holds its
PM's slot two stages longer — through Ship Planning and Ship Complete. Nobody has
asked for the cap to change and it was left alone; if PMs start hitting `asap_limit`
sooner than they used to, that is why, and the fix is a stage filter on the count
