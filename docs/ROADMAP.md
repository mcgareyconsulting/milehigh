...
- 2026-09-23 · build · src design/release-mobile-recommendations.md — **release becomes a PAGE for subs** (`/sub/releases/:id`, pushed from a Job Log card or a to-do; back restores the log's scroll). Two-row sticky header (back · number chip · stage chip · ⋯ with Copy link / Close), sticky tab strip **Details · Activity · Attachments** — Issues, Splices and Change Log are not rendered (staff-only by the 9/07 decisions). Details = the spec's 44px key/value rows (Schedule, Details, the sub's to-dos here), read-only. **Activity** = the desktop rail's rows through the SAME `buildTimeline()` transform, fed by `GET /brain/subcontractor/releases/<id>/activity` which serves `SUB_ACTIVITY_ACTIONS` only (staff ACTIVITY_ACTIONS minus fab-order and the issue register, plus `update_notes`), newest first, with the **note composer pinned** — `POST .../notes` goes through `UpdateNotesCommand` (the one writer of `Releases.notes`) with `external_user_id="sub:<id>"`, and `_resolve_event_user_names` now resolves that prefix so the staff rail shows the sub's name instead of "Brain" (the 9/07 resolver note, closed). **Attachments is a reader**: drawings (family versions, current flagged) and photos as rows, a drawing opens a full-screen pdf.js canvas reader (fit-width × DPR, zoom buttons, page stepper) streamed through sub-scoped file routes; photos open full-screen. **Spec items deliberately not built**: Take photo / Upload (`uploaded_by_user_id` is a NOT NULL users FK — the slice-4 authorship wall), pins and any markup authoring, Carmen, Procore/Trello links, stage picker (stage is display-only for subs — open question 3 answered conservatively), Splices tab. Open questions 1–2 default to *no phone markup authoring* and *no Splices for subs* until Bill weighs in
- 2026-09-23 · decision · src — — **Daniel reversed three of those defaults the same day**: (1) subs **upload photos** — Take photo (camera) / Upload photo on the Attachments tab through `POST /brain/subcontractor/releases/<id>/photos`; needed the authorship wall to move, so `ReleasePhoto.uploaded_by_user_id` goes nullable with a new `uploaded_by_subcontractor_id` and a one-uploader CHECK (migration `add_subcontractor_photo_uploads.py`, NOT yet run), and `UploadPhotoCommand` stamps `external_user_id="sub:<id>"` on the event; (2) subs **change stage** from the release page — the stage chip opens a picker limited server-side to `SUB_STAGES` = Ship Complete → Install Start → Install Complete → Complete (field stages only; shop stages refused with 400), running the full `UpdateStageCommand` cascade with the same `sub:<id>` attribution (new optional `external_user_id` on the command); (3) the **Splices tab** is on the sub page — the release family with crew/stage/date/hours, rows on the sub's crew tappable, other crews listed but inert. Also: the Details action pair was dropped — Add note lives on Activity, photos on Attachments. Drawing upload and markup authoring remain desktop-only
- 2026-09-23 · decision · src — — **Splices tab removed again for subs** (Daniel: "they just see what they see"); a sub on a splice still sees the original's drawings / PDF pack because Attachments lists the release FAMILY's versions. **Sub editability is now fixed at four things**: stage (field stages), notes, photos, file attachments — a PDF from "Upload file" becomes the release's next drawing version through the same commands the staff hub uses, so `ReleaseDrawingVersion.uploaded_by_user_id` went nullable with `uploaded_by_subcontractor_id` alongside the photo change (migration renamed to `add_subcontractor_uploads.py`, covers both tables, NOT yet run). Nothing else on the release is editable by a sub. The sub shell is now `position: fixed; inset: 0` so the document can never scroll and the top bar is locked on every tab
- 2026-09-23 · build · src — — **employees get the same phone shell** at `/m/*` (`StaffMobileShell`, rendered by `AppShell` in place of its desktop chrome, inside the same providers): **To-Dos** (shared `TodosMobile` body over the staff `/todos` + `/notifications` routes; a release opens the normal `ReleaseHubModal`), **Job Log** = the day calendar with ONE lane select — *Shipping Planning* first (every crew's releases in the Ship Planning stage; new `stage=` filter on `/brain/install-schedule/by-day`, plus `month=`) then each installer crew — and **T&M** (the existing `TMTickets` page). A phone landing on `/` goes to `/m/todos`; the account sheet has a *Desktop site* link back. The staff modal keeps every action a PM has at a desk
- 2026-09-24 · decision · src client contact sheet (4 subs) — **sub login scope is the COMPANY, not one crew.** The client's contact sheet made the gap concrete: Saul Rodriguez (A&N) runs Saul 1, 2 and 3, and the account model held one `installer_team`. Now `scope_clause()` in `app/brain/sub_portal/service.py` reads the account's `company_name` through the SAME `SUB_COMPANY_CREWS` table Invoice Paid uses (`company_for_installer` in one direction, new `crew_stems_for_company` in the other), so **email → account → company → every crew with that stem → releases** and the invoicing tab can never disagree with the login about who owns Saul 3. Matching is suffix-insensitive (`company_key`: "S&S Construction" == "S&S Construction, LLC") and crew matching is case-insensitive on the stem. An account whose company is NOT in the table falls back to its admin-picked `installer_team`; with neither, nothing (fail closed, unchanged). `GET /brain/subcontractor/installer-teams` now returns the resolved crew list and the shell shows it under *Viewing as*. Also added `Subcontractor.phone` (migration `add_phone_to_subcontractors.py`, NOT yet run) on the invite form, roster and account sheet. The four real accounts (A&N / ACDLC / S&S / Apogee) are to be created through the roster invite flow once prod is migrated — company names must match the map
- 2026-09-24 · decision · src — — **the T13 department photo gate is enforced on the sub portal too** (Daniel: "yes, we will want to enforce the gate on the subs view"). Merging #388 turned the gate ON, and a sub moving Ship Planning → Install Start now owes the *Ship Complete* photo. `PATCH /brain/subcontractor/releases/<id>/stage` answers the same `422 photo_required` (`stage` + `requested_stage`) and accepts `gate_exception_note`; `POST .../photos` takes the `stage` tag (validated against `STAGE_TO_GROUP`). The phone gate sheet (`SubStageGateSheet`) offers the desktop dialog's two exits — take/upload a photo tagged with the gate stage, or a written reason — and retries the same change. Same flow on the employee phone page through the staff adapter
