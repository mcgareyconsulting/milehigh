...
diagnosed), BUG-13 (low, reproduce first), BUG-14 (low, **needs a physical
iPad** — don't hand this to an agent), BUG-17 (**blocked** on Daniel's stub).
...
| BUG-17 | To-Do page cleanup [daniel-2026-08-29]. **Not agent-ready — blocked on Daniel's `.jsx`/`.tsx` stub from another app** (see Owed). Current page is 411 lines: admin sees all assigned to-dos with status/owner/due-bucket/item-type/job/text filters, non-admins see only their own, grouped into collapsible due-urgency sections. Scope the cleanup against the stub before touching it — the server-side scoping invariant (non-admins can only ever see or modify their own items) is load-bearing and must survive any rewrite | `frontend/src/pages/ToDos.jsx`, `frontend/src/services/todosApi.js` | med |
...
| **To-Do page stub** *(from Daniel, not Bill)* — a working `.jsx`/`.tsx` to-do implementation from another app, to scope the cleanup against | BUG-17 — it is the whole gate | 2026-08-29 |
