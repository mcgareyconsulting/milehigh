...
| BUG-17 | To-Do page cleanup [daniel-2026-08-29]. ~~Blocked on Daniel's stub~~ **unblocked 2026-08-29** — reference delivered (HPB EOS To-Dos page, 44 files) and scoped in **`docs/design/todos-page/README.md`**; read that before touching anything. Headline from the triage: **we have no to-do table** — ours are `ChecklistItem` rows created only by the meeting extractor, with no create / delete / archive / field-edit path, so the reference's entire write model has no counterpart and "port the page" is not a coherent instruction. The doc splits it into take-now (three-tone due urgency, expandable rows, muted completed state, counts on tabs — all frontend, no decisions), needs-a-backend-touch (owner grouping as a toggle; widen `PATCH /todos/<id>` beyond `status`), and decide-first (standalone to-dos, archive, rich text, per-row visibility). **First slice = Tier 1.** Invariant that must survive: non-admins can only ever see or modify their own items, enforced server-side | `frontend/src/pages/ToDos.jsx`, `frontend/src/services/todosApi.js`, `app/brain/todos_routes.py`; scope doc `docs/design/todos-page/README.md`; reference tree `~/Desktop/Reference/eos-todos-reference/` (outside git) | med |
