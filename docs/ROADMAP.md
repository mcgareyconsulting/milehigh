...
**BUG-19 added and built 2026-09-03** (branch `fix/asap-behavior`) — the ASAP
half of the date rules, a direct follow-on to BUG-11. **BUG-20 found by prod
audit and half-built the same day** on the same branch.
...
- 2026-09-03 · decision · src daniel-2026-09-03 — third input, and the one that simplifies the ruleset most: **ASAP is a colour, not a date** (BUG-19). Every date in the install column is now one a person typed — there is no synthetic date in the system for the ruleset to have to explain, and the fieldops §5.4 line ("projected carry no colour, yellow = a hard green date now past due") extends cleanly with "red = a hard date flagged ASAP"
...
| daniel-2026-09-03 | session notes (no transcript) | Daniel's ASAP rework, taken live 2026-09-03 — BUG-19, plus the "treat ASAP as a hard date everywhere" call on the downstream classifiers. No line anchors; cited by slug only |
