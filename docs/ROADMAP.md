...
project: MHMW
updated: 2026-08-29
verified: origin/main @ 1beba8d (PR #352 Bill answers)
...
trigger now settled, fully spec'd), BUG-16 (high, client-asked), BUG-18 (low,
diagnosed), BUG-17 (med, scoped — Tier 1 is
decision-free), BUG-13 (low, reproduce first), BUG-14 (low, **needs a physical
iPad** — don't hand this to an agent).

| ID | Fix | Entry point | Pri |
|---|---|---|---|
| BUG-11 | Hard-date color dump fires too early — moves from the ship stages to **`Install Start` or later** (Bill reversed his own 8/15 rule: *"I might have given you bad information"* [bill-2026-08-21#L109]; color survives ship stages, dumps once install starts [#L115]; yellow overdue must never silently disappear [#L113]). ~~Open build call: trigger = the `Install Start` stage vs. the `start_install` date arriving~~ **decided 2026-08-29 (Daniel), stated exactly:** the flip fires on a **stage transition whose destination is `Install Start` or any later stage** — i.e. destination ∈ {`Install Start`, `Install Complete`, `Complete`}. **Never** the `start_install` date arriving: a date-driven dump fires on schedule even when the install slipped, which is exactly the silently-vanishing yellow Bill ruled out [#L113]. **Mind the two look-alike names** — `start_install` is the date *field*, `Install Start` is the *stage value*; the roadmap's earlier "start-install trigger" phrasing conflated them. **"Or later" is load-bearing:** a release that jumps Ship Planning → `Complete` and skips `Install Start` must still dump, which a bare equality check on `Install Start` would miss. Accepted cost: if the crew never moves the stage, the color outlives the real start | `app/brain/job_log/features/start_install/shipping_stage_date_discipline.py` (+ `stage/command.py` intercept); tests `tests/brain/test_shipping_stage_date_discipline.py` | **high — client-decided** |
| BUG-16 | DWL **HOLD** must drop when a submittal's ball-in-court changes [daniel-2026-08-29]. A HOLD set against one drafter is stale the moment the submittal moves to someone else, so the DWL shows held work that isn't. Clear `submittal_drafting_status` back to `''` on a real BIC change. **Two call sites, not one** — the webhook path and the reconciliation/audit path both write `ball_in_court` independently (the same second-caller shape as BUG-8/BUG-9); fix both or it comes back on the next audit sweep. Emit a `SubmittalEvents` row so the drop is auditable rather than silent | webhook: `app/procore/procore.py:1190` (`if db_ball_value != webhook_ball_value:`); audit: `app/procore/procore.py:2209`; statuses: `app/brain/drafting_work_load/engine.py:42` + `routes.py:297` (`VALID_DRAFTING_STATUSES`) | **high — client-asked** |
| BUG-17 | To-Do page cleanup [daniel-2026-08-29]. ~~Blocked on Daniel's stub~~ **unblocked 2026-08-29** — reference delivered (HPB EOS To-Dos page, 44 files) and scoped in **`docs/design/todos-page/README.md`**; read that before touching anything. Headline from the triage: **we have no to-do table** — ours are `ChecklistItem` rows created only by the meeting extractor, with no create / delete / archive / field-edit path, so the reference's entire write model has no counterpart and "port the page" is not a coherent instruction. **Scope narrowed the same day (Daniel): frontend / QoL only, no backend behavior.** In scope, seven items, all in `ToDos.jsx` — three-tone due urgency (we lack the 14-day amber band), view-first rows that expand `detail` on title click, a muted completed state, tab counts, board-column + empty-state chrome, owner initials, and an owner-grouping **toggle** beside the existing due buckets (verified frontend-only: `to_dict` already returns `owner_name`, `models.py:1694`). Out of scope and recorded as such: widening `PATCH /todos/<id>` past `status`, standalone to-dos, archive, rich text, per-row visibility. Invariant that must survive: non-admins can only ever see or modify their own items, enforced server-side | `frontend/src/pages/ToDos.jsx`, `frontend/src/services/todosApi.js`, `app/brain/todos_routes.py`; scope doc `docs/design/todos-page/README.md`; reference tree `~/Desktop/Reference/eos-todos-reference/` (outside git) | med |
| BUG-18 | Left icon rail pops in extra rows on load [daniel-2026-08-29]. **Diagnosed:** `isAdmin` and `canSeeReport` both initialise `false` and are set only after an async `checkAuth()` round-trip (`AppShell.jsx:43-44`, `:66-74`), so an admin first paints the non-admin subset and then gains Subs / Meetings / Board / T&M / Matching / Invoicing. Worse than a plain pop-in: `ITEM_H` is computed from viewport height against row count, so adding ~6 rows **re-lays out the whole rail**, not just appends to it. Two candidate fixes — cache the last-known role in `localStorage` (the rail already does this for open state under `RAIL_KEY`) and hydrate optimistically, or reserve the role-gated rows' space until auth resolves. Icons themselves are inline SVG path strings, so nothing is being fetched | `frontend/src/components/AppShell.jsx:43-74`, `frontend/src/components/Rail.jsx` (`ITEM_H_MAX`/`ITEM_H_MIN`, `RAIL_KEY`) | low — cosmetic |
...
**Delivered 2026-08-29:** the **To-Do page reference** — the HPB EOS standalone To-Dos page (44 files), which unblocked BUG-17 the same day. Parked outside git at `~/Desktop/Reference/eos-todos-reference/`; scoped in `docs/design/todos-page/README.md`.
...
| eos-todos-2026-08-29 | `~/Desktop/Reference/eos-todos-reference/` | The HPB EOS app's standalone To-Dos page, delivered by Daniel 2026-08-29 as `eos-todos-reference.zip` (44 files, Next.js/Firestore). **Outside the repo on purpose** — same convention as the transcripts, and this repo is public while that is another app's source. Scoped for our use in `docs/design/todos-page/README.md` |
